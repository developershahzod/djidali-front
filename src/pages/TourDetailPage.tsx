...
const TourDetailPage: React.FC = () => {
  ...
  const itineraryItems = useMemo<ItineraryItem[]>(() => {
    ...
    if (tour.itinerary && typeof tour.itinerary === 'object' && !Array.isArray(tour.itinerary)) {
      return Object.entries(tour.itinerary).map(([title, description], index) => ({
        dayNumber: index + 1,
        title: title,
  ...
  return (
    <>
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-between px-[clamp(20px,3.47vw,50px)] py-[clamp(40px,5.56vw,80px)]">
          ...
          <div className="flex flex-col gap-[40px] mt-auto">
            ...
            <div className="flex justify-end">
              <Link to="#location" className="text-white underline text-[20px]" style={{ letterSpacing: '-0.4px' }}>
                {tour.destination || tour.location || t('tourDetail.location')}
              </Link>
            </div>
          ...
          <div className="flex flex-col gap-[clamp(20px,2.78vw,40px)] w-full md:w-[426px]">
            ...
            <div className="flex flex-col gap-[10px] text-white">
              <div className="text-[clamp(32px,3.47vw,50px)] font-medium leading-[1]" style={{ letterSpacing: '-1px' }}>
                {tour.duration}{' '}
                <span className="text-[clamp(24px,2.78vw,40px)] font-extralight">{translate('tourDetail.days')}</span>
              </div>
              <div className="text-[clamp(16px,1.39vw,20px)] font-light leading-[1.4]" style={{ letterSpacing: '-0.4px' }}>
      ...
      <section className="relative bg-white" style={{ height: '394px' }}>
        ...
        <div className="absolute top-[80px] left-[50px] right-[50px] flex justify-between items-center max-w-full">
          <div className="flex flex-col gap-[10px] w-[234px]">
            <div className="text-[20px] leading-[24px]" style={{ letterSpacing: '-0.4px' }}>{translate('tourDetail.people')}</div>
          ...
          <div className="flex flex-col gap-[10px] w-[234px]">
            <div className="text-[20px] leading-[24px]" style={{ letterSpacing: '-0.4px' }}>{translate('tourDetail.minAge')}</div>
            <div className="text-[32px] font-medium leading-[40px]" style={{ letterSpacing: '-0.64px' }}>{(tour as any).minAge ? `${(tour as any).minAge} yoshdan` : t('tourDetail.minAgeValue')}</div>
          </div>
          <div className="w-[62px] h-[1px] bg-gray-300 rotate-90" />
          <div className="flex flex-col gap-[10px] w-[234px]">
            <div className="text-[20px] leading-[24px]" style={{ letterSpacing: '-0.4px' }}>{translate('tourDetail.tourType')}</div>
            <div className="text-[32px] font-medium leading-[40px] capitalize" style={{ letterSpacing: '-0.64px' }}>
              {(tour.category as any)?.name || t('tourDetail.ecotourism')}
            </div>
          </div>
          <div className="w-[62px] h-[1px] bg-gray-300 rotate-90" />
          <div className="flex flex-col gap-[10px] w-[234px]">
            <div className="text-[20px] leading-[24px]" style={{ letterSpacing: '-0.4px' }}>{translate('tourDetail.difficulty')}</div>
            <div className="text-[32px] font-medium leading-[40px]" style={{ letterSpacing: '-0.64px' }}>{(tour as any).difficulty || t('tourDetail.easy')}</div>
          </div>
        </div>
      ...
      <section className="px-[50px] py-[80px] max-w-[1440px] mx-auto">
        ...
        {(tour.inclusions && tour.inclusions.length > 0 ? tour.inclusions : [t('tourDetail.accommodation'), t('tourDetail.meals'), t('tourDetail.guideServices'), t('tourDetail.transfer'), t('tourDetail.excursions')]).map((item, i) => (
      ...
      <div className="w-[426px] pl-8 border-l border-gray-200">
            ...
            {(tour.exclusions && tour.exclusions.length > 0 ? tour.exclusions : [t('tourDetail.personalExpenses'), t('tourDetail.alcohol'), t('tourDetail.insurance'), t('tourDetail.huntingEquipment')]).map((item, i) => (
              <div key={i} className="text-[20px] text-[#333333] leading-[24px] mb-[16px] pl-[30px] relative" style={{ letterSpacing: '-0.4px' }}>
                <span className="absolute left-[10px]">•</span>
                {item}
              </div>
