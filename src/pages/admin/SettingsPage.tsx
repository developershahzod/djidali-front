...
const AdminSettingsPage = () => {
  ...
  const handleSiteChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name.startsWith('social_')) {
      const socialKey = name.replace('social_', '') as keyof SiteSettings['socialLinks'];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      localStorage.setItem('djidali_hero_settings', JSON.stringify(heroSettings));
      localStorage.setItem('djidali_site_settings', JSON.stringify(siteSettings));
  ...
  const sections = [
    { key: 'hero' as const, label: 'Hero баннер', icon: <ImageIcon className="h-4 w-4" /> },
    { key: 'site' as const, label: 'Общие настройки', icon: <Palette className="h-4 w-4" /> },
  ];
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="flex-1">
          {activeSection === 'hero' && (
            <div className="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-lg backdrop-blur">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-[#2F2A24]">Настройки Hero баннера</h3>
                  <p className="text-sm text-[#8E7A5E]">Управление главным баннером на главной странице</p>
                </div>
                {/* Language tabs */}
                ...
                >
                  ...
                  <div className="relative flex h-full flex-col items-center justify-center p-6 text-center text-white">
                    <h2 className="mb-2 text-2xl font-bold">
                      {heroSettings[`title${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}` as keyof HeroSettings] || 'Заголовок'}
                    </h2>
                    <p className="text-sm opacity-90">
                      {heroSettings[`subtitle${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}` as keyof HeroSettings] || 'Подзаголовок'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={heroSettings[`title${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}` as keyof HeroSettings] as string}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    value={heroSettings[`subtitle${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}` as keyof HeroSettings] as string}
                    onChange={handleHeroChange}
                    rows={2}
                    className="w-full rounded-xl border border-[#E2D5C1] bg-white px-4 py-2.5 text-[#2F2A24] focus:border-[#BFA480] focus:outline-none focus:ring-2 focus:ring-[#BFA480]/30"
                ...
                <div>
                  <label className="mb-1.5 flex items-center justify-between text-sm font-medium text-[#6B5B4C]">
                    ...
                    <span className="text-[#2F2A24]">{heroSettings.overlayOpacity}%</span>
                  </label>
                  <input
                    type="range"
                    name="overlayOpacity"
          ...
          {activeSection === 'site' && (
            <div className="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-lg backdrop-blur">
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-[#2F2A24]">Общие настройки сайта</h3>
                <p className="text-sm text-[#8E7A5E]">Базовая информация о сайте и контакты</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="border-t border-[#E2D5C1] pt-4">
                  <h4 className="mb-4 text-sm font-semibold text-[#2F2A24]">Социальные сети</h4>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-[#6B5B4C]">
                        Telegram
