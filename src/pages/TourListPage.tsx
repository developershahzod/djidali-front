...
export interface TourFilters {
  ...
  durationFrom?: string;
  durationTo?: string;
  sortBy?: 'price-asc' | 'price-desc' | 'duration-asc' | 'duration-desc' | 'popular';
}
...
const TourListPage: React.FC = () => {
  ...
  const [activeFilters, setActiveFilters] = useState<TourFilters | null>(() => {
    const hasUrlParams = searchParams.get('destination') ||
  ...
  const { tours: rawTours, loading, error, pagination } = useTours({
    page: currentPage,
  ...
  const tours = React.useMemo(() => {
    ...
    if (frontendFilters?.priceFrom && frontendFilters.priceFrom !== '') {
      ...
      if (!isNaN(priceFrom)) {
        filtered = filtered.filter(tour => {
          const price = typeof tour.price === 'object' ? (tour.price as any)?.amount : tour.price;
    ...
    if (frontendFilters?.durationFrom && frontendFilters.durationFrom !== '') {
      ...
      if (!isNaN(durationFrom)) {
        filtered = filtered.filter(tour => (tour.duration || 0) >= durationFrom);
    ...
    if (frontendFilters?.durationTo && frontendFilters.durationTo !== '') {
      ...
      if (!isNaN(durationTo)) {
        filtered = filtered.filter(tour => (tour.duration || 0) <= durationTo);
    ...
    if (frontendFilters?.sortBy && frontendFilters.sortBy !== 'popular') {
      filtered = [...filtered].sort((a, b) => {
        const priceA = typeof a.price === 'object' ? (a.price as any)?.amount : a.price || 0;
        const priceB = typeof b.price === 'object' ? (b.price as any)?.amount : b.price || 0;
