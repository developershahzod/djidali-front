...
const TourEditModal: React.FC<TourEditModalProps> = ({
  ...
  const formatDateForInput = (dateValue?: any): string => {
    ...
    try {
      ...
      // Handle object format from API (e.g., { date: "...", timezone: "..." })
      if (typeof dateValue === "object" && dateValue !== null) {
        const dateObj = dateValue as Record<string, any>;
        console.log("Date object keys:", Object.keys(dateObj));

        // Try common date string properties first
        const possibleDateString =
          dateObj.date ||
          dateObj.value ||
          dateObj.iso ||
          dateObj.dateTime ||
        ...
        if (possibleDateString && typeof possibleDateString === "string") {
          const parsed = new Date(possibleDateString);
          if (!isNaN(parsed.getTime())) {
            return parsed.toISOString().split("T")[0];
          }
        }

        // Try passing object directly to Date (works if it has valueOf or toString)
        const directDate = new Date(dateObj as any);
        if (!isNaN(directDate.getTime())) {
          return directDate.toISOString().split("T")[0];
        }

        console.error("Could not parse date object:", dateObj);
        return "";
      }
      ...
      console.error("Date parsing error:", e, "Input:", dateValue);
      return "";
    }
  };
  ...
  useEffect(() => {
    const openEditModal = async (tourSummary: ExtendedApiTour) => {
      ...
      try {
        ...
        toast.error({
          title: "Loading Error",
          message: "Could not load tour details for editing. Please try again.",
        });
        onClose();
        return;
      }
      ...
      // Debug: Log full tour object to see date structure
      console.log("Full tour object:", JSON.stringify(tour, null, 2));
      console.log(
        "Tour startDate:",
        tour.startDate,
        "Type:",
        typeof tour.startDate,
      );
      console.log("Tour endDate:", tour.endDate, "Type:", typeof tour.endDate);

      // Extract dates - handle Prisma DateTime objects or strings
      const extractDateValue = (dateField: any): string => {
        if (!dateField) return "";
        // If it's already a string (ISO format)
        if (typeof dateField === "string") return dateField;
        // If it's a Date object
        if (dateField instanceof Date) return dateField.toISOString();
        // If it's an object (Prisma might return objects)
        if (typeof dateField === "object") {
          // Try common patterns
          if (dateField.toISOString) return dateField.toISOString();
          if (dateField.date) return dateField.date;
          if (dateField.value) return dateField.value;
          if (dateField.iso) return dateField.iso;
          // Last resort - stringify and check if it looks like a date
          const str = JSON.stringify(dateField);
          console.log("Date object stringified:", str);
      ...
      const rawStartDate =
        extractDateValue(tour.startDate) ||
        extractDateValue((tour as any).start_date) ||
        extractDateValue((tour as any).startAt);
      const rawEndDate =
        extractDateValue(tour.endDate) ||
        extractDateValue((tour as any).end_date) ||
        extractDateValue((tour as any).endAt);

      console.log("Extracted rawStartDate:", rawStartDate);
      console.log("Extracted rawEndDate:", rawEndDate);
      ...
      setTourFormData({
        ...
        maxParticipants:
          tour.maxParticipants || (tour as any).max_participants || 10,
        startDate: formatDateForInput(rawStartDate),
        endDate: formatDateForInput(rawEndDate),
        ...
        categoryId:
          ...
          (categoryData && categoryData.id ? String(categoryData.id) : ""),
      });
  ...
  const handleImageUpload = async (files: FileList | null) => {
    ...
    if (oversizedFiles.length > 0) {
      ...
      toast.warning({
        ...
        message: `The following files exceed the maximum size of ${MAX_FILE_SIZE_MB}MB: ${fileNames}. Please choose smaller files.`,
      });
      return;
    }
  ...
  const removeItineraryStep = (index: number) => {
    setTourFormData((prev) => ({
      ...prev,
      itinerary:
        prev.itinerary.filter((_, i) => i !== index).length > 0
  ...
  const handleSave = async () => {
    ...
    const itineraryPayload = tourFormData.itinerary.reduce(
      (acc, step, index) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {categories.map((category) => (
                    <option key={category.id} value={category.id}>
                      {typeof category.name === "string"
                        ? category.name
                        : (category.name as any)[activeLangTab] ||
              ...
              <div>
                ...
                <input
                  ...
                  min={tourFormData.startDate || getTodayDate()}
                  onChange={(e) =>
                    setTourFormData({
                      ...tourFormData,
            ...
            <div className="space-y-4">
              ...
              {(tourFormData.images.length > 0 || previewImages.length > 0) && (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {[...new Set([...tourFormData.images, ...previewImages])].map(
                    (image, index) => (
                      <div
                        key={`${image}-${index}`}
