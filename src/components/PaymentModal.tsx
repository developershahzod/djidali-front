...
const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const [_selectedMethod, _setSelectedMethod] = useState<PaymentMethod>("card");
  ...
  const _handleCardNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const _handleExpiryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const _handleCvvChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await clickPaymentService.createClickOrder(
        orderId,
        amountInUzs,
      );
      ...
      setIsProcessing(false);
      toast.error({
        title: t("paymentModal.error"),
        message:
          error instanceof Error
            ? error.message
            : t("paymentModal.unknownError"),
      });
    }
  };
        ...
        <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-10 text-center transform transition-all animate-in">
          ...
          <div className="bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 rounded-2xl p-6">
            ...
            <p className="text-4xl font-bold bg-gradient-to-r from-[#8f7b49] to-[#a08957] bg-clip-text text-transparent">
              ...
              <span className="text-2xl">UZS</span>
            </p>
          </div>
        </div>
      </div>
