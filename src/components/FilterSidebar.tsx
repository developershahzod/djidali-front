...
const FilterSidebar: React.FC<FilterSidebarProps> = ({ filters, onFiltersChange }) => {
  ...
  const [expandedSections, setExpandedSections] = useState<string[]>(['duration', 'price']);
      ...
      <div className="border-b border-gray-100 mb-5 pb-5 last:border-0">
        ...
        >
          <div className="flex items-center space-x-2">
            <span className="font-bold text-gray-900 group-hover:text-emerald-600 transition-colors text-sm">{title}</span>
  ...
  return (
    <div className="w-full lg:w-80 bg-white lg:border-r border-gray-100 lg:min-h-screen overflow-y-auto p-4 lg:p-6 shadow-sm lg:sticky lg:top-0 rounded-xl lg:rounded-none mb-6 lg:mb-0">
      ...
      <FilterSection id="duration" title={t('filters.durationFilter')}>
        <div className="space-y-4">
          <div className="flex items-center justify-between text-sm text-gray-600">
            <span className="bg-emerald-50 px-3 py-1 rounded-full font-medium text-emerald-700">{filters.duration[0]} {t('filters.days')}</span>
            <span className="text-gray-400">—</span>
            <span className="bg-emerald-50 px-3 py-1 rounded-full font-medium text-emerald-700">{filters.duration[1]} {t('filters.days')}</span>
          </div>
          <div className="relative px-3 py-2">
            <input
              ...
              onInput={(e) => {
                ...
                onFiltersChange({
                  duration: [parseInt(target.value), Math.max(parseInt(target.value), filters.duration[1])]
                });
              }}
          ...
          <div className="relative px-3 py-2">
            <label className="block text-xs text-gray-500 mb-1">{t('filters.maxDuration')}</label>
            <input
              type="range"
              min="1"
              max="30"
              ...
              onInput={(e) => {
                const target = e.target as HTMLInputElement;
                onFiltersChange({
                  duration: [Math.min(filters.duration[0], parseInt(target.value)), parseInt(target.value)]
                });
              }}
              className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer range-slider"
              style={{
      ...
      <FilterSection id="price" title={t('filters.priceFilter')}>
        <div className="space-y-4">
          <div className="flex items-center justify-between text-sm text-gray-600">
            <span className="bg-emerald-50 px-3 py-1 rounded-full font-medium text-emerald-700">{filters.priceRange[0].toLocaleString()} {t('filters.som')}</span>
            <span className="text-gray-400">—</span>
            <span className="bg-emerald-50 px-3 py-1 rounded-full font-medium text-emerald-700">{filters.priceRange[1].toLocaleString()} {t('filters.som')}</span>
          </div>
          <div className="relative px-3 py-2">
            <input
              type="range"
              ...
              onInput={(e) => {
                const target = e.target as HTMLInputElement;
                onFiltersChange({
                  priceRange: [parseInt(target.value), Math.max(parseInt(target.value), filters.priceRange[1])]
                });
              }}
              className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer range-slider"
              style={{
          ...
          <div className="relative px-3 py-2">
            ...
            <input
              ...
              onInput={(e) => {
                const target = e.target as HTMLInputElement;
                onFiltersChange({
                  priceRange: [Math.min(filters.priceRange[0], parseInt(target.value)), parseInt(target.value)]
                });
              }}
              className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer range-slider"
              style={{
        ...
        <div className="space-y-4">
          <div className="text-center">
            <span className="bg-amber-50 px-4 py-2 rounded-full font-medium text-amber-700">
              {filters.rating === 0 ? t('filters.allRatings') : `${filters.rating}+ ${t('filters.stars')}`}
            </span>
          </div>
          <div className="px-2">
            <input
        ...
        <div className="space-y-3">
          <label className="flex items-center space-x-3 p-3 bg-red-50 rounded-lg hover:bg-red-100 transition-colors cursor-pointer">
            <input
              ...
              onChange={(e) => onFiltersChange({ discountOnly: e.target.checked })}
              className="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-500"
            />
            <span className="text-sm font-medium text-red-700">{t('filters.discountOnly')}</span>
          </label>
          ...
          <label className="flex items-center space-x-3 p-3 bg-green-50 rounded-lg hover:bg-green-100 transition-colors cursor-pointer">
            <input
              ...
              onChange={(e) => onFiltersChange({ guaranteedOnly: e.target.checked })}
              className="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500"
            />
            <span className="text-sm font-medium text-green-700">{t('filters.guaranteedOnly')}</span>
          </label>
        </div>
      </FilterSection>
      ...
      <FilterSection id="language" title={t('filters.language')}>
        <div className="space-y-2">
          {[
            ...
            <label key={lang.id} className="flex items-center space-x-3 p-2 hover:bg-gray-50 rounded-lg cursor-pointer">
              <input 
                type="checkbox" 
