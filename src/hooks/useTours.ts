...
const getLocalizedText = (content: any, lang: string): string => {
  ...
  if (
    content.titleUz !== undefined ||
    content.nameUz !== undefined ||
    content.descriptionUz !== undefined
  ) {
    // Determine the field prefix
    if (
      content.titleUz !== undefined ||
      content.titleRu !== undefined ||
      content.titleEng !== undefined ||
      content.titleDe !== undefined
    ) {
      return getTranslatedField(content, "title", lang);
    }
    if (
      content.nameUz !== undefined ||
      content.nameRu !== undefined ||
      content.nameEng !== undefined ||
      content.nameDe !== undefined
    ) {
    ...
    if (
      ...
      content.descriptionEng !== undefined ||
      content.descriptionDe !== undefined
    ) {
  ...
  const normalized = Object.entries(content).reduce<Record<string, string>>(
    (acc, [key, value]) => {
      if (value == null) {
        return acc;
      }
...
const convertApiTourToTour = (apiTour: any, lang: string): Tour => {
  ...
  const safeDateConvert = (
    dateValue: string | Record<string, any> | undefined,
  ): string | null => {
  ...
  if (typeof priceSource === "number") {
    ...
    priceValue =
      Number((priceSource as any).amount) ||
      Number((priceSource as any).uzs) ||
      Number((priceSource as any).UZS) ||
      Number((priceSource as any).usd) ||
      Number((priceSource as any).value) ||
      0;
  }
  ...
  const itinerary: Tour["itinerary"] = programDays.map(
    (item: any, idx: number) => {
      // Safely get title - ensure it's always a string
      let itemTitle = getTranslatedField(item, "title", lang);
      if (!itemTitle && item.title) {
        itemTitle =
          typeof item.title === "string"
            ? item.title
            : getLocalizedText(item.title, lang) || `Day ${idx + 1}`;
      }
      if (!itemTitle) itemTitle = `Day ${idx + 1}`;

      // Safely get description - ensure it's always a string
      let itemDescription = getTranslatedField(item, "description", lang);
    ...
    return arr
      .map((item) => {
        ...
        if (typeof item === "object" && item !== null) {
          // The lang code for english is 'en', but the api returns 'eng'
          return (
            item[lang] ||
            item.eng ||
            item.ru ||
  ...
  // Safely get tour title - ensure it's always a string
  let tourTitle = getTranslatedField(apiTour, "title", lang);
  if (!tourTitle && apiTour.title) {
    tourTitle =
      typeof apiTour.title === "string"
        ? apiTour.title
        : getLocalizedText(apiTour.title, lang) || "Untitled Tour";
  }
  if (!tourTitle) tourTitle = "Untitled Tour";

  // Safely get tour description - ensure it's always a string
  let tourDescription = getTranslatedField(apiTour, "description", lang);
  if (!tourDescription && apiTour.description) {
    tourDescription =
      typeof apiTour.description === "string"
        ? apiTour.description
        : getLocalizedText(apiTour.description, lang) || "";
  }
  ...
  return {
    ...
    category: apiTour.category
      ? {
          ...
          name: (() => {
            const catName = getTranslatedField(apiTour.category, "name", lang);
            if (catName) return catName;
            if (apiTour.category.name) {
              return typeof apiTour.category.name === "string"
                ? apiTour.category.name
                : getLocalizedText(apiTour.category.name, lang) || "Category";
            }
            return "Category";
          })(),
          slug: apiTour.category.slug || "tour",
          description: (() => {
            const catDesc = getTranslatedField(
              apiTour.category,
              "description",
              lang,
            );
            if (catDesc) return catDesc;
            if (apiTour.category.description) {
              return typeof apiTour.category.description === "string"
                ? apiTour.category.description
                : getLocalizedText(apiTour.category.description, lang) || "";
            }
            return "";
          })(),
          ...
          created_at:
            safeDateConvert(apiTour.category.createdAt) ?? createdAtIso,
          updated_at:
            safeDateConvert(apiTour.category.updatedAt) ?? updatedAtIso,
        }
      : undefined,
    images: images,
    dates: tourDates,
...
export const useTours = (params: UseToursParams = {}): UseToursReturn => {
  ...
  const [pagination, setPagination] =
    useState<UseToursReturn["pagination"]>(null);
  ...
  const fetchTours = async () => {
    try {
      ...
      if (apiParams.destination)
        requestParams.destination = apiParams.destination;
      if (apiParams.minPrice || apiParams.min_price)
        requestParams.minPrice = apiParams.minPrice || apiParams.min_price;
      if (apiParams.maxPrice || apiParams.max_price)
        requestParams.maxPrice = apiParams.maxPrice || apiParams.max_price;
      ...
      if (apiParams.category_id)
        requestParams.categoryId = apiParams.category_id.toString();
      ...
      const convertedTours = response.data.map((tour) =>
        convertApiTourToTour(tour, language),
      );
      ...
      setPagination({
        ...
        to: Math.min(
          (response.page || 1) * requestParams.limit,
